...

{
	...
	ProtocMsgBase<Client>::registerCBFun(PROTOCO_NAME(message::ClientInit),                      &Client::parseClientInit);

	ProtocMsgBase<Client>::registerCBFun(PROTOCO_NAME(message::MapScreenRefreshCharacter),        &Client::parseClientChar);
...

{
	...
	message::MapScreenRefreshCharacter* msg = static_cast<message::MapScreenRefreshCharacter*>(p);
...

	{
...

		{
			...
			const message::MapCharInfo& m = msg->mapinfo();
...

{
	...
	message::ClientInit* msg = static_cast<message::ClientInit*>(p);

	if (msg)

	{

		if (message::Response_NewChar == msg->result())

		{

			message::CreateChar m;

			m.set_cityid(2);

			m.set_mapid(1);

			message::CharInfo* pinfo = m.mutable_charinfo();
			...
			pinfo->set_chartype(message::enumChar_Person);
