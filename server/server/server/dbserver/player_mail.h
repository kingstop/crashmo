...
struct MailData
{
	...
	u32 guid;
	std::vector<message::PlayerMail*> mails;
};
class MailManager
{
public:
	...
	void deleteMail(u32 guid, message::DelMail* p);
	void readMail(u32 guid, message::ReadMail* p);
	void addMail(const std::string& name, u32 guid, message::UserSendMailByid* p);
protected:
