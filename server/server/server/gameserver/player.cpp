...
//#include "FRMap.h"
/*
Player::Player(Session* p, const message::PlayerInfo* info)
...
void Player::onCheckNewPlayer()
{
    ...
    message::PlayerSSData* p = m_playerInfo.mutable_ss_data();
    const message::PlayerSCData& scd = m_playerInfo.sc_data();
    u32 flag = p->flag();
...
{
    const message::PlayerSCData& scd = m_playerInfo.sc_data();
    const message::PlayerSSData& ssd = m_playerInfo.ss_data();
...
void Player::changeScene(u32 cityID, u32 sceneID, float posX, float posY, float dir)
{
	...
	if(gGSWorld.isRunCity(cityID))
	{
		...
		{
			::message::PlayerSCData* psc = m_playerInfo.mutable_sc_data();
			psc->set_city(cityID);
	...
	}else
	{
		...
		psc->set_posy(posY);
		psc->set_dir(dir);
		message::MsgGS2DBChangeServer msg;
		msg.set_city(cityID);
...
void Player::moveMsgRequired(message::CharMove* msg)
{
    if (msg)
    {
...
void Player::onJoinScene()
{
	if (_map)
	{
		message::MapScreenRefreshCharacter refreshmsg;
		...
		sendMsg(&refreshmsg);
		Player* target = NULL;
		message::NineScreenRefreshPlayer msg;
