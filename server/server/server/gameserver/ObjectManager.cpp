...
Player* ObjectManager::createPlayer(Session* s, const message::PlayerInfo* p)
{
    if (!s || !p )
    {   return NULL ;}
    ...
    if (NULL == pkPlayer)
    {
        pkPlayer = Memory::createObject<Player, Session*, const message::PlayerInfo*>(s, p);
...
void ObjectManager::savePlayerinfo(const message::PlayerInfo& info, message::LineState state)
{
    message::MsgGS2DBSavePlaye msg;
    message::DBPlayerData* p = msg.add_players();
    p->set_state(state);
    *(p->mutable_data()) = info;
...
void ObjectManager::saveAllPlayers()
{
    ...
    for (it; it!= itend ; ++it)
    {
        ...
        {
            message::DBPlayerData* p = msg.add_players();
            p->set_state(message::_ON_LINE_);
            const message::PlayerInfo& info = pkPlayer->getPlayerInfo();
            *(p->mutable_data()) = info;
            ++ count;
           // Mylog::log_player(LOG_DEBUG, "save player [%u]", info.sc_data().guid(), info.sc_data().posx(), info.sc_data().posy());
...
void ObjectManager::removePlayer(Player* p)
{
    ...
    const message::PlayerInfo& info = p->getPlayerInfo();
    u32 g =  info.sc_data().guid();
    u32 a = info.ss_data().account();
    savePlayerinfo(info, message::_OFF_LINE);
